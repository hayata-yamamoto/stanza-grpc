...
def _init_route(server: grpc.Server) -> None:
    stanza_pb2_grpc.add_StanzaServicer_to_server(stanza_service.StanzaService(Env.LANGUAGE),
                                                 server)
    health_pb2_grpc.add_HealthServicer_to_server(health_service.HealthService(),
                                                 server)
